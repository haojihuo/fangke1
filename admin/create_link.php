...
<td><button type="button" class="secondary action-btn" onclick="openModal('visitor-modal-<?= $row['id'] ?>')">访客数据</button></td>
</tr>
<?php endforeach; ?>
</tbody></table>
</div>
...
<div id="visitor-modal-<?= $row['id'] ?>" class="modal-overlay">
  <div class="modal-card">
    <div class="modal-head"><h3>访客数据（链接ID：<?= $row['id'] ?>）</h3><button type="button" class="modal-close" onclick="closeModal(this)">×</button></div>
