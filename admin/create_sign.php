...
<tr><td><?= $t['id'] ?></td><td><?= $t['temp_link_id'] ?></td><td><?= $t['total_times'] ?></td><td><?= $t['start_at'] ?> ~ <?= $t['end_at'] ?></td><td><a href="<?= htmlspecialchars($url) ?>" target="_blank"><?= htmlspecialchars($url) ?></a><br><img src="<?= $qr ?>" width="80"></td><td><a href="sign_data.php?task_id=<?= $t['id'] ?>"><button type="button" class="secondary action-btn">签到数据</button></a></td></tr>
<?php endforeach; ?>
</tbody></table></div>
</div>
